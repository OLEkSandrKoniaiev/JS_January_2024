...
function rectangular_area(height, width) {
    return height * width;
}
...
function cylinder_volume(height, radius) {
...
function paragraph_create(text) {
...
function create_list_static(text) {
    document.write(`
        <ul>
            <li>${text}</li>
            <li>${text}</li>
...
function create_list(text, count = 1) {
    document.write("<ul>");
...
function create_primary_list(arr) {
    document.write("<ul>");
    for (let element of arr) {
...
function swap(arr, index1, index2, is_return = false) {
    let temp = arr[index1];
...
let my_arr_3 = ["asd", 45, true, -9.2, 'C', 8, 12, "wewe", false];
